---
title: "bmore_arduino_text_to_sheets"
...
```
# R Function to Read Text Files, Process Data, & Write Hourly and Daily Averages to Google Sheets

## Function Inputs
* Raw (Text) Data Folder: https://drive.google.com/drive/folders/194ncM5sbPbsphw0rEhQhPvHTbbPp_ifi?usp=drive_link
* Target Google Drive Folder
* Number of Files to Write

## Function Outputs
* Google sheet with data format matching sample: https://docs.google.com/spreadsheets/d/17Vbd4FFzBijvWdVH4z56M_iWWdRQbYU0LyU_niB65sI/edit?gid=1199813510#gid=1199813510
* Sheets include: "About AQI", "PM2.5", "PM10", "Ozone", "Carbon-Dioxide", "About the project"
* Data Include: Hourly and Daily Averages

## How the Function Works!
- Reads raw text file in the data folder
- Identifies text files by file ID to address potential duplication
- Process data to compute hourly and daily averages of the parameters
- Creates an empty google spreadsheet in the target folder
- Renames the google spreadsheet with the name of the txt file in the data folder
- Writes the specified sheet names and transfers data
- Ensures no duplication by only writing sheetnames that do not exist in the target folder

### Requirements
- Need to create the target google drive folder and copy folder ID
- Need to have the data folder ID
- Need to be logged in to the google account hosting the target folder

### Disclaimer! 

Google sheets API does not allow conditional formating and color/table formating, needs to be done manually
...
# Write (n) Files to target Google Drive Folder
```{r}
...
```
# Write All Files to target Google Drive Folder
